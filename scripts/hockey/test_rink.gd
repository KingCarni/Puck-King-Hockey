...
func _create_placeholder_players() -> void:
	var home_player: MeshInstance3D = _create_cylinder("HomePlayerPlaceholder", 0.42, 1.1, Vector3(-7.0, 0.65, -1.25))
	home_player.material_override = _make_material(Color(0.12, 0.34, 1.0, 1.0), 0.0, 0.35)
	world.add_child(home_player)

	var away_player: MeshInstance3D = _create_cylinder("AwayPlayerPlaceholder", 0.42, 1.1, Vector3(7.0, 0.65, 1.25))
...
func _create_placeholder_puck() -> void:
	var puck: MeshInstance3D = _create_cylinder("PuckPlaceholder", 0.24, 0.12, Vector3(0.0, 0.18, 0.0))
